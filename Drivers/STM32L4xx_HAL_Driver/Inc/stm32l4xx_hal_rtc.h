...
typedef struct {
    ...
    uint32_t SubSeconds; /*!< Specifies the RTC_SSR RTC Sub Second register content.
                              ...
                              If the free running 32 bit counter is not activated (mode binary none)
                                 - This parameter corresponds to a time unit range between [0-1] Second with [1 Sec /
                            SecondFraction +1] granularity else
    ...
    uint32_t DayLightSaving; /*!< This interface is deprecated. To manage Daylight Saving Time, please use
                                HAL_RTC_DST_xxx functions */

    uint32_t StoreOperation; /*!< This interface is deprecated. To manage Daylight Saving Time, please use
                                HAL_RTC_DST_xxx functions */
} RTC_TimeTypeDef;
...
typedef struct {
    ...
    uint32_t SubSeconds; /*!< Specifies the RTC_SSR RTC Sub Second register content.
                            ...
                            SecondFraction +1] granularity else
                                  - This parameter corresponds to the free running 32 bit counter. */
    uint32_t
        BinaryAutoClr; /*!< Clear synchronously counter (RTC_SSR) on binary alarm.
    ...
    uint8_t AlarmDateWeekDay; /*!< Specifies the RTC Alarm Date/WeekDay.
                                   If the Alarm Date is selected, this parameter must be set to a value in the 1-31
                                 range. If the Alarm WeekDay is selected, this parameter can be a value of @ref
...
#if defined(STM32L4P5xx) || defined(STM32L4Q5xx)
/** @defgroup RTC_Input_parameter_format_definitions RTC input or output data format for date (Year, Month, Weekday) and
 * time (Hours, Minutes, Seconds). Warning : It Should not be confused with the Binary mode @ref RTCEx_Binary_Mode.
 * @{
 */
#define RTC_FORMAT_BIN                                                                                               \
    0x00000000u /* This parameter will trigger a SW conversion to fit with the native BCD format of the HW Calendar. \
                   It should not be confused with the Binary mode @ref RTCEx_Binary_Mode. */

#define RTC_FORMAT_BCD                               \
    0x00000001u /* Native format of the HW Calendar. \
                   It should not be confused with the Binary mode @ref RTCEx_Binary_Mode. */
/**
 * @}
 */
...
#define RTC_ALARMSUBSECONDMASK_SS14_3                                                     \
    (RTC_ALRMASSR_MASKSS_0 | RTC_ALRMASSR_MASKSS_1) /*!< SS[14:3] are don't care in Alarm \
...
#define RTC_ALARMSUBSECONDMASK_SS14_5                                                     \
    (RTC_ALRMASSR_MASKSS_0 | RTC_ALRMASSR_MASKSS_2) /*!< SS[14:5] are don't care in Alarm \
                                                         comparison. Only SS[4:0] are compared  */
#define RTC_ALARMSUBSECONDMASK_SS14_6                                                     \
    (RTC_ALRMASSR_MASKSS_1 | RTC_ALRMASSR_MASKSS_2) /*!< SS[14:6] are don't care in Alarm \
...
#define RTC_ALARMSUBSECONDMASK_SS14_9                                                     \
    (RTC_ALRMASSR_MASKSS_0 | RTC_ALRMASSR_MASKSS_3) /*!< SS[14:9] are don't care in Alarm \
                                                         comparison. Only SS[8:0] are compared  */
#define RTC_ALARMSUBSECONDMASK_SS14_10                                                     \
    (RTC_ALRMASSR_MASKSS_1 | RTC_ALRMASSR_MASKSS_3) /*!< SS[14:10] are don't care in Alarm \
...
#define RTC_ALARMSUBSECONDMASK_SS14_12                                                     \
    (RTC_ALRMASSR_MASKSS_2 | RTC_ALRMASSR_MASKSS_3) /*!< SS[14:12] are don't care in Alarm \
...
#define IS_RTC_OUTPUT(OUTPUT)                                                                                  \
    (((OUTPUT) == RTC_OUTPUT_DISABLE) || ((OUTPUT) == RTC_OUTPUT_ALARMA) || ((OUTPUT) == RTC_OUTPUT_ALARMB) || \
     ((OUTPUT) == RTC_OUTPUT_WAKEUP) || ((OUTPUT) == RTC_OUTPUT_TAMPER))
#else
#define IS_RTC_OUTPUT(OUTPUT)                                                                                  \
    (((OUTPUT) == RTC_OUTPUT_DISABLE) || ((OUTPUT) == RTC_OUTPUT_ALARMA) || ((OUTPUT) == RTC_OUTPUT_ALARMB) || \
...
#if defined(STM32L412xx) || defined(STM32L422xx) || defined(STM32L4P5xx) || defined(STM32L4Q5xx)
#define IS_RTC_OUTPUT_PULLUP(TYPE) (((TYPE) == RTC_OUTPUT_PULLUP_NONE) || ((TYPE) == RTC_OUTPUT_PULLUP_ON))
#endif

#define IS_RTC_OUTPUT_REMAP(REMAP) (((REMAP) == RTC_OUTPUT_REMAP_NONE) || ((REMAP) == RTC_OUTPUT_REMAP_POS1))
