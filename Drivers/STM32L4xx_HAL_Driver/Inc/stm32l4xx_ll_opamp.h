...
typedef struct {
    uint32_t
        PowerMode; /*!< Set OPAMP power mode.
    ...
    uint32_t
        FunctionalMode; /*!< Set OPAMP functional mode by setting internal connections: OPAMP operation in standalone,
                           follower, ... This parameter can be a value of @ref OPAMP_LL_EC_FUNCTIONAL_MODE
                             @note If OPAMP is configured in mode PGA, the gain can be configured using function @ref
                           LL_OPAMP_SetPGAGain().
    ...
    uint32_t InputInverting; /*!< Set OPAMP inverting input connection.
                                  ...
                                  @note OPAMP inverting input is used with OPAMP in mode standalone or PGA with external
                                capacitors for filtering circuit. Otherwise (OPAMP in mode follower), OPAMP inverting
                                input is not used (not connected to GPIO pin), this parameter is discarded.
...
#define LL_OPAMP_POWERSUPPLY_RANGE_LOW \
    0x00000000U /*!< Power supply range low. On STM32L4 series: Vdda lower than 2.4V. */
#define LL_OPAMP_POWERSUPPLY_RANGE_HIGH \
    (OPAMP1_CSR_OPARANGE) /*!< Power supply range high. On STM32L4 series: Vdda higher than 2.4V. */
/**
 * @}
 */
...
#define LL_OPAMP_POWERMODE_LOWPOWER \
    (OPAMP_POWERMODE_LPOTR_REGOFFSET | OPAMP_CSR_OPALPM) /*!< OPAMP power mode low-power */

#define LL_OPAMP_POWERMODE_NORMAL \
    LL_OPAMP_POWERMODE_NORMALPOWER /*!< OPAMP power mode normal - Old Naming for compatibility */
...
#define LL_OPAMP_MODE_FOLLOWER \
    (OPAMP_CSR_OPAMODE_1 | OPAMP_CSR_OPAMODE_0) /*!< OPAMP functional mode, OPAMP operation in follower */
...
#define LL_OPAMP_INPUT_NONINVERT_IO0 \
    0x00000000U /*!< OPAMP non inverting input connected to GPIO pin (pin PA0 for OPAMP1, pin PA6 for OPAMP2) */
#define LL_OPAMP_INPUT_NONINV_DAC1_CH1 \
    (OPAMP1_CSR_VPSEL) /*!< OPAMP non inverting input connected to DAC1 channel1 output */
/**
 * @}
 */
...
#define LL_OPAMP_INPUT_INVERT_IO0                                                                                      \
    0x00000000U /*!< OPAMP inverting input connected to GPIO pin (valid also in PGA mode for filtering). Note: OPAMP   \
                   inverting input is used with OPAMP in mode standalone or PGA with external capacitors for filtering \
                   circuit. Otherwise (OPAMP in mode follower), OPAMP inverting input is not used (not connected to    \
                   GPIO pin). */
#define LL_OPAMP_INPUT_INVERT_IO1                                                                                    \
    (OPAMP_CSR_VMSEL_0) /*!< OPAMP inverting input (low leakage input) connected to GPIO pin (available only on      \
                           package BGA132). Note: OPAMP inverting input is used with OPAMP in mode standalone or PGA \
                           with external capacitors for filtering circuit. Otherwise (OPAMP in mode follower), OPAMP \
                           inverting input is not used (not connected to GPIO pin). */
#define LL_OPAMP_INPUT_INVERT_CONNECT_NO                                                                            \
    (OPAMP_CSR_VMSEL_1) /*!< OPAMP inverting input not externally connected (intended for OPAMP in mode follower or \
...
#define LL_OPAMP_TRIMMING_PMOS \
    (OPAMP_OTR_TRIMOFFSETP | OPAMP1_CSR_CALSEL) /*!< OPAMP trimming of transistors differential pair PMOS */
/**
 * @}
 */
 ...
 */
#if defined(OPAMP1) && defined(OPAMP2)
#define __LL_OPAMP_IS_ENABLED_ALL_COMMON_INSTANCE() (LL_OPAMP_IsEnabled(OPAMP1) | LL_OPAMP_IsEnabled(OPAMP2))
#else
#define __LL_OPAMP_IS_ENABLED_ALL_COMMON_INSTANCE() (LL_OPAMP_IsEnabled(OPAMP1))
#endif
...
/** @defgroup OPAMP_LL_EF_Configuration_opamp_common Configuration of OPAMP hierarchical scope: common to several OPAMP
 * instances
 * @{
 */
