...
#define IS_SMBUS_NO_STRETCH(STRETCH) (((STRETCH) == SMBUS_NOSTRETCH_DISABLE) || ((STRETCH) == SMBUS_NOSTRETCH_ENABLE))
...
#define IS_SMBUS_TRANSFER_MODE(MODE)                                                                                  \
    (((MODE) == SMBUS_RELOAD_MODE) || ((MODE) == SMBUS_AUTOEND_MODE) || ((MODE) == SMBUS_SOFTEND_MODE) ||             \
     ...
     ((MODE) == (SMBUS_AUTOEND_MODE | SMBUS_SENDPEC_MODE)) || ((MODE) == (SMBUS_AUTOEND_MODE | SMBUS_RELOAD_MODE)) || \
...
HAL_StatusTypeDef
HAL_SMBUS_Slave_Transmit_IT(SMBUS_HandleTypeDef* hsmbus, uint8_t* pData, uint16_t Size, uint32_t XferOptions);
HAL_StatusTypeDef
HAL_SMBUS_Slave_Receive_IT(SMBUS_HandleTypeDef* hsmbus, uint8_t* pData, uint16_t Size, uint32_t XferOptions);
