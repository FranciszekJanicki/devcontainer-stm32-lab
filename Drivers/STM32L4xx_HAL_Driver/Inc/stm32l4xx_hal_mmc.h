...
#define HAL_MMC_SECURE_ERASE \
    0x80000000U /*!< Perform a secure purge according SRT on the erase groups identified by CMD35 & 36 */
...
#define HAL_MMC_SECURE_TRIM_STEP2 \
    0x80008000U /*!< Perform a secure purge according SRT on the write blocks previously identified    */

#define IS_MMC_ERASE_TYPE(TYPE)                                                              \
    (((TYPE) == HAL_MMC_ERASE) || ((TYPE) == HAL_MMC_TRIM) || ((TYPE) == HAL_MMC_DISCARD) || \
...
#define HAL_MMC_SRT_WRITE_CHAR_ERASE \
    0x00000002U /*!< Information removed by an overwriting with a character followed by an erase                    */
#define HAL_MMC_SRT_WRITE_CHAR_COMPL_RANDOM \
    0x00000004U /*!< Information removed by an overwriting with a character, its complement then a random character */
...
#define IS_MMC_SRT_TYPE(TYPE)                                                     \
    (((TYPE) == HAL_MMC_SRT_ERASE) || ((TYPE) == HAL_MMC_SRT_WRITE_CHAR_ERASE) || \
     ((TYPE) == HAL_MMC_SRT_WRITE_CHAR_COMPL_RANDOM) || ((TYPE) == HAL_MMC_SRT_VENDOR_DEFINED))
/**
 * @}
 */
#endif /* defined(STM32L4P5xx) || defined(STM32L4Q5xx) || defined(STM32L4R5xx) || defined(STM32L4R7xx) || \
          defined(STM32L4R9xx) || defined(STM32L4S5xx) || defined(STM32L4S7xx) || defined(STM32L4S9xx) */
...
HAL_StatusTypeDef
HAL_MMC_ReadBlocks_IT(MMC_HandleTypeDef* hmmc, uint8_t* pData, uint32_t BlockAdd, uint32_t NumberOfBlocks);
HAL_StatusTypeDef
HAL_MMC_WriteBlocks_IT(MMC_HandleTypeDef* hmmc, uint8_t* pData, uint32_t BlockAdd, uint32_t NumberOfBlocks);
/* Non-Blocking mode: DMA */
HAL_StatusTypeDef
HAL_MMC_ReadBlocks_DMA(MMC_HandleTypeDef* hmmc, uint8_t* pData, uint32_t BlockAdd, uint32_t NumberOfBlocks);
HAL_StatusTypeDef
HAL_MMC_WriteBlocks_DMA(MMC_HandleTypeDef* hmmc, uint8_t* pData, uint32_t BlockAdd, uint32_t NumberOfBlocks);
...
HAL_StatusTypeDef
HAL_MMC_RegisterCallback(MMC_HandleTypeDef* hmmc, HAL_MMC_CallbackIDTypeDef CallbackId, pMMC_CallbackTypeDef pCallback);
 ...
 */
HAL_StatusTypeDef
HAL_MMC_EraseSequence(MMC_HandleTypeDef* hmmc, uint32_t EraseType, uint32_t BlockStartAdd, uint32_t BlockEndAdd);
