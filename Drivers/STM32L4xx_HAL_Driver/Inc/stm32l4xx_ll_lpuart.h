...
#define __LL_LPUART_DIV(__PERIPHCLK__, __BAUDRATE__)                                                          \
    (uint32_t)(((((uint64_t)(__PERIPHCLK__) * LPUART_LPUARTDIV_FREQ_MUL) + (uint32_t)((__BAUDRATE__) / 2U)) / \
                (__BAUDRATE__)) &                                                                             \
               LPUART_BRR_MASK)
#endif /* USART_PRESC_PRESCALER */
...
LL_LPUART_ConfigCharacter(USART_TypeDef* LPUARTx, uint32_t DataWidth, uint32_t Parity, uint32_t StopBits)
{
...
#if defined(USART_PRESC_PRESCALER)
__STATIC_INLINE void
LL_LPUART_SetBaudRate(USART_TypeDef* LPUARTx, uint32_t PeriphClk, uint32_t PrescalerValue, uint32_t BaudRate)
#else
