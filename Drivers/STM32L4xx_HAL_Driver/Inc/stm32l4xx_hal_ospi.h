...
#define HAL_OSPI_STATE_HYPERBUS_INIT \
    ((uint32_t)0x00000001U) /*!< Initialization done in hyperbus mode but timing configuration not done */
#define HAL_OSPI_STATE_READY \
    ((uint32_t)0x00000002U) /*!< Driver ready to be used                                                */
...
#define HAL_OSPI_STATE_BUSY_CMD \
    ((uint32_t)0x00000008U) /*!< Command without data on-going                                          */
#define HAL_OSPI_STATE_BUSY_TX \
    ((uint32_t)0x00000018U) /*!< Indirect Tx on-going                                                   */
#define HAL_OSPI_STATE_BUSY_RX \
    ((uint32_t)0x00000028U) /*!< Indirect Rx on-going                                                   */
#define HAL_OSPI_STATE_BUSY_AUTO_POLLING \
...
#define HAL_OSPI_DELAY_BLOCK_BYPASSED \
...
#define HAL_OSPI_ALTERNATE_BYTES_DTR_ENABLE \
    ((uint32_t)OCTOSPI_CCR_ABDTR) /*!< DTR mode enabled for alternate bytes phase  */
/**
 * @}
 */
...
#define HAL_OSPI_SIOO_INST_ONLY_FIRST_CMD \
    ((uint32_t)OCTOSPI_CCR_SIOO) /*!< Send instruction only for the first command */
/**
 * @}
 */
...
#define HAL_OSPI_AUTOMATIC_STOP_DISABLE \
    ((uint32_t)0x00000000U) /*!< AutoPolling stops only with abort or OSPI disabling */
#define HAL_OSPI_AUTOMATIC_STOP_ENABLE \
    ((uint32_t)OCTOSPI_CR_APMS) /*!< AutoPolling stops as soon as there is a match       */
/**
 * @}
 */
...
#define HAL_OSPI_TIMEOUT_COUNTER_DISABLE \
    ((uint32_t)0x00000000U) /*!< Timeout counter disabled, nCS remains active               */
#define HAL_OSPI_TIMEOUT_COUNTER_ENABLE \
    ((uint32_t)OCTOSPI_CR_TCEN) /*!< Timeout counter enabled, nCS released when timeout expires */
/**
 * @}
 */
 ...
 * @{
 */
#define HAL_OSPI_FLAG_BUSY \
    OCTOSPI_SR_BUSY /*!< Busy flag: operation is ongoing */
#define HAL_OSPI_FLAG_TO \
    OCTOSPI_SR_TOF /*!< Timeout flag: timeout occurs in memory-mapped mode */
#define HAL_OSPI_FLAG_SM \
    OCTOSPI_SR_SMF /*!< Status match flag: received data matches in autopolling mode */
#define HAL_OSPI_FLAG_FT \
    OCTOSPI_SR_FTF /*!< Fifo threshold flag: Fifo threshold reached or data left after read from memory is complete */
#define HAL_OSPI_FLAG_TC                                                                                            \
    OCTOSPI_SR_TCF /*!< Transfer complete flag: programmed number of data have been transferred or the transfer has \
                      been aborted */
#define HAL_OSPI_FLAG_TE \
    OCTOSPI_SR_TEF /*!< Transfer error flag: invalid address is being accessed */
/**
 * @}
 */
...
#define IS_OSPI_FREE_RUN_CLK(CLK) (((CLK) == HAL_OSPI_FREERUNCLK_DISABLE) || ((CLK) == HAL_OSPI_FREERUNCLK_ENABLE))

#define IS_OSPI_CLOCK_MODE(MODE) (((MODE) == HAL_OSPI_CLOCK_MODE_0) || ((MODE) == HAL_OSPI_CLOCK_MODE_3))
...
#define IS_OSPI_DATA_DTR_MODE(MODE) (((MODE) == HAL_OSPI_DATA_DTR_DISABLE) || ((MODE) == HAL_OSPI_DATA_DTR_ENABLE))
...
#define IS_OSPI_DLYBYP(MODE) (((MODE) == HAL_OSPI_DELAY_BLOCK_USED) || ((MODE) == HAL_OSPI_DELAY_BLOCK_BYPASSED))
#if defined(OCTOSPI_DCR3_MAXTRAN)
...
#define IS_OSPIM_IO_PORT(PORT)                                                                                         \
    (((PORT) == HAL_OSPIM_IOPORT_NONE) || ((PORT) == HAL_OSPIM_IOPORT_1_LOW) || ((PORT) == HAL_OSPIM_IOPORT_1_HIGH) || \
     ((PORT) == HAL_OSPIM_IOPORT_2_LOW) || ((PORT) == HAL_OSPIM_IOPORT_2_HIGH))
